...
# from kitti_object_eval_python.non_max_suppression.nms_gpu import rotate_iou_gpu_eval
...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_gt):
        ...
        if difficulty >= 3:
            pass
            # if ((gt_anno["occluded"][i] > MAX_OCCLUSION[-1])
            #         or (gt_anno["truncated"][i] > MAX_TRUNCATION[-1])
            ...
            if difficulty > 0 and separate:
                if not ((gt_anno["occluded"][i] > MAX_OCCLUSION[difficulty - 1])
                    or (gt_anno["truncated"][i] > MAX_TRUNCATION[difficulty - 1])
                        or (height <= MIN_HEIGHT[difficulty - 1])):
                    ignore = True
    ...
    for i in range(num_dt):
        ...
        if difficulty in [0, 1, 2] and height < MIN_HEIGHT[difficulty]:
            ignored_dt.append(1)
            # pass
        # elif not difficulty in [0, 1, 2] and height < MIN_HEIGHT[-1]:
        #     # ignored_dt.append(1)
        #     pass
        elif valid_class == 1:
            ignored_dt.append(0)
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    # print(difficulty)
    # print(np.sum([len(ign) - len(ign[ign != 0]) for ign in ignored_gts]))
...
def eval_class(gt_annos,
    ...
    """
    assert len(gt_annos) == len(dt_annos), f"{len(gt_annos)}, {len(dt_annos)}"
    num_examples = len(gt_annos)
    split_parts = get_split_parts(num_examples, num_parts)
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                thresholds, recalls = get_thresholds(thresholdss, total_num_valid_gt)
                thresholds = np.array(thresholds)
                recalls = np.array(recalls)
                recall[m, l, k, :len(recalls)] = recalls
...
def filter_det_range(dets, close, far):
    dets = deepcopy(dets)
    valid_idx = (dets['location'][:, 2] > close) * (dets['location'][:, 2] <= far)
    for k in dets:
...
def get_official_eval_result(gt_annos,
    ...
    overlap_easy = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.5, 0.5, 0.5],
                             ...
                             [0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5]])
    overlap_diff = np.array([[0.9, 0.7, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7],
                            [0.9, 0.7, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7],
                            [0.9, 0.7, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7]])
    # Yurong
    # overlap = []
    # for iou in np.arange(0.05, 1, 0.05):
    #     overlap.append(overlap_mod.copy())
    #     overlap[-1][1, 0] = iou
    #     overlap[-1][2, 0] = iou
    # min_overlaps = np.stack(overlap, axis=0)
    min_overlaps = np.stack([overlap_diff, overlap_mod, overlap_easy], axis=0)  # [2, 3, 5]
    # min_overlaps = overlap_mod.reshape(1, *overlap_mod.shape)
    # Yurong End
    class_to_name = {
        0: 'Car',
        1: 'Pedestrian',
    ...
    dt_annos = [filter_det_range(dets, close, far) for dets in dt_annos]
    gt_annos = [filter_det_range(dets, close, far) for dets in gt_annos]
...
def get_range_eval_result(gt_annos,
    ...
    overlap_mod = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7],
                            ...
                            [0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7]])
    overlap_easy = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.5, 0.5, 0.5],
    #overlap_easy = np.array([[0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5],
                             [0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5],
                             [0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5]])
    # Yurong
    # overlap = []
    ...
    for i in range(len(ranges)-1):
        ...
        dt_annos_range = [filter_det_range(
            dets, ranges[i], ranges[i+1]) for dets in dt_annos]
        gt_annos_range = [filter_det_range(
            dets, ranges[i], ranges[i+1]) for dets in gt_annos]

        metrics = do_eval_v3_bev3d(
            gt_annos_range,
            dt_annos_range,
            current_classes,
            min_overlaps,
            ...
            z_axis=z_axis,
            z_center=z_center)
        bev07.append(get_mAP_v2(metrics["bev"]["precision"][0, :, 0])[0])
        threeD07.append(get_mAP_v2(metrics["3d"]["precision"][0, :, 0])[0])
        bev05.append(get_mAP_v2(metrics["bev"]["precision"][0, :, 1])[0])
        threeD05.append(get_mAP_v2(metrics["3d"]["precision"][0, :, 1])[0])
    result_line = "IoU 0.7:\n"
    result_line += "RANGE " + "  ".join([f"{x:> 6d}" for x in ranges[1:]]) + "\n"
    result_line += "BEV:  " + ", ".join([f"{x:6.3f}" for x in bev07]) + "\n"
    result_line += "3D :  " + ", ".join([f"{x:6.3f}" for x in threeD07]) + "\n"
    result_line += "IoU 0.5:\n"
    result_line += "RANGE " + "  ".join([f"{x:> 6d}" for x in ranges[1:]]) + "\n"
    result_line += "BEV:  " + ", ".join([f"{x:6.3f}" for x in bev05]) + "\n"
    result_line += "3D :  " + ", ".join([f"{x:6.3f}" for x in threeD05]) + "\n"
    result_line += "IoU 0.7:\n"
    result_line += ", ".join([f"{x:3.1f} / {y:3.1f}" for x, y in zip(bev07, threeD07)]) + "\n"
    result_line += "IoU 0.5:\n"
    result_line += ", ".join([f"{x:3.1f} / {y:3.1f}" for x, y in zip(bev05, threeD05)]) + "\n"
    ret = {}
    ret['range'] = ranges
    ret['bev07'] = bev07
    ret['threeD07'] = threeD07
    ret['bev05'] = bev05
    ret['threeD05'] = threeD05
    return result_line, ret
